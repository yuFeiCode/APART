...
Dis.figures.path.top1 = './Dis_6.1_figures/top1_sets/'
Dis.figures.path.top3 = './Dis_6.1_figures/top3_sets/'
...
get.SAT.result.only.for.actionable.warning = function(all_eval_releases, LLM.result.dir, SAT.result.dir, SATname, line.ground.truth, lineLevelMetrics, save.fig.dir,IFA_y_limit, FPavg_y_limit, top_n, Dis.result.dir)
{
  ...
  SAT.result.df = select(SAT.result.df, element)
  SAT_F.result.df = select(SAT_F.result.df, element)
  SAT_glance.lr.df = select(SAT_glance.lr.result.df, element)
  SAT_ngram.result.df = select(SAT_ngram.result.df, element)
  SAT_linedp.result.df = select(SAT_linedp.result.df, element)
  ...
  all_tools_results <- list(
    ...
    SAT_glance.lr.df,
    SAT_ngram.result.df,
    SAT_linedp.result.df,
    SAT_deeplinedp.result.df,
    SAT_LLM4SA.result.df
  )
  ...
  write.csv(overall_result, file = paste0(Dis.result.dir, SATname, '/', SATname, '_overall_result.csv'))
}
